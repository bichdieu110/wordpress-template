	...
	function get_most_viewed( $mode = '', $limit = 10, $chars = 0, $display = true ) {
		...
		if ( $most_viewed->have_posts() ) {
          ...
          // print_r($categories[0]);
				...
				$temp = str_replace( '%POST_DATE%', get_the_time( get_option( 'date_format' ) ), $temp );
				$temp = str_replace( '%POST_TIME%', get_the_time( get_option( 'time_format' ) ), $temp );
				$temp = str_replace( '%POST_THUMBNAIL%', get_the_post_thumbnail( null,'large',true ), $temp);
				$temp = str_replace( '%POST_CATEGORY_ID%', $post_category_id, $temp );
				$temp = str_replace( '%POST_CATEGORY_NAME%', $post_category_name, $temp );
				$temp = str_replace( '%POST_AUTHOR%', get_the_author(), $temp );
				$temp = str_replace( '%POST_TAGS_URL%', $taglink, $temp );
        $temp = str_replace( '%POST_TAGS_NAME%', $tagname, $temp );


        //Post Tags
        $posttags = get_the_tags(get_the_ID());

        $taglist = '<ul class="c-articles_item_tags">';

        if ($posttags) {
          foreach($posttags as $tag) {
            $taglist .= '<li class="c-articles_item_tags_tag u-bg_cream"><a href="' . get_tag_link($tag->term_id) . '">'.$tag->name.'</a></li>';
          }
        }
        $taglist .= '</ul>';

        $temp = str_replace( '%POST_TAGS_LIST%', $taglist, $temp );
        $output .= $temp;
			}
...
### Class: WP-PostViews Widget
 class WP_Widget_PostViews extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		echo '<ul class="c-articles c-articles-ranking">'."\n";
		switch($type) {
			case 'least_viewed':
    ...
    echo '<li class="c-articles_item_ranking u-pc-only">
            <img class="c-articles_item_ranking_img" src="'.URL_IMAGE.'/common/ranking.png" alt="">
          </li>';
		echo '</ul>'."\n";
